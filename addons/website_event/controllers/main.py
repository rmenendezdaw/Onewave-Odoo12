...
class WebsiteEventController(http.Controller):
    ...
    @http.route(['''/event/<model("event.event", "[('website_id', 'in', (False, current_website_id))]"):event>/registration/confirm'''], type='json', auth="public", methods=['POST'], website=True)
    def registration_confirm(self, event, **post):
        ...
        diccionario = {
            '1-email': user.email,
            '1-name': user.name,
            '1-phone':  post['1-phone'],
            '1-ticket_id': post['1-ticket_id']
        }
        ...
        emails = Attendees.sudo().search([('email', '=', user.email), ('event_id', '=', event.id)])
        if (len(emails) > 0):
            return {'res': 'False'}

        registrations = self._process_registration_details(diccionario)
